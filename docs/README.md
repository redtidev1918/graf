# Graf 文档中心

Graf 是一个极简、**自托管的 Markdown 发布平台**，**兼容 Telegra.ph（Telegraph API）**，
整个服务跑在 **Cloudflare Workers + D1** 上：发布即得一个短链页面，可选段落级评论。

> 在线示例：[graf.redtidev1918.workers.dev](https://graf.redtidev1918.workers.dev)
>
> GitHub 仓库：[redtidev1918/graf](https://github.com/redtidev1918/graf)

## 📥 下载 grafctl

`grafctl` 是发布 / 管理 CLI（单二进制，含签名校验 `checksums.txt`）：

| 平台 | 文件 |
|---|---|
| macOS | `grafctl_<version>_darwin_amd64|arm64.tar.gz` / `.zip` |
| Linux | `grafctl_<version>_linux_amd64|arm64.tar.gz` / `.zip` |
| Windows | `grafctl_<version>_windows_amd64|arm64.tar.gz` / `.zip` |

👉 最新版见 [GitHub Releases](https://github.com/redtidev1918/graf/releases/latest)。

## 按任务找文档

| 我想…… | 请看 |
|---|---|
| 了解 Graf 的起源与演变 | [项目历史与血统](HISTORY.zh-CN.md) |
| 对接 Telegraph API 客户端 | [API 参考](API.zh-CN.md) |
| 理解代码结构与运行方式 | [架构说明](ARCHITECTURE.zh-CN.md) |
| 部署到 Cloudflare 或自托管 | [部署指南](DEPLOYMENT.zh-CN.md) |

## 全部文档

### 中文

- [API 参考](API.zh-CN.md)
- [架构说明](ARCHITECTURE.zh-CN.md)
- [部署指南](DEPLOYMENT.zh-CN.md)
- [项目历史与血统](HISTORY.zh-CN.md)

### English

- [API Reference](API.md)
- [Architecture](ARCHITECTURE.md)
- [Deployment Guide](DEPLOYMENT.md)
- [Project History](HISTORY.md)

## 其他入口

- [GitHub 仓库](https://github.com/redtidev1918/graf)
- [项目说明（README）](https://github.com/redtidev1918/graf/blob/master/README.md)
- [部署指南](DEPLOYMENT.zh-CN.md)
- [变更记录](https://github.com/redtidev1918/graf/blob/master/CHANGELOG.zh-CN.md)
